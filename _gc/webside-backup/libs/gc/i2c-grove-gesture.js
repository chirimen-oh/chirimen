...
PAJ7620.prototype = {
  ...
  init: function(){
    ...
    return new Promise(function(resolve, reject){
      self.i2cPort.open(self.slaveAddress).then(function(i2cSlave){
        self.i2cSlave = i2cSlave;
        console.log("i2cPort.open");
        var thread = (function* () {
          i2cSlave.write8(0xef,0);  // BankSelect=0
        ...
        Promise.all([
          i2cSlave.read8(0,true),
          i2cSlave.read8(1,true)
        ]).then(function(v){
          if((v[0] != 0x20)||(v[1] != 0x76)){
            console.log("read error!");
            reject(v);
          }else{
            var thread2 = (function* () {
              for(var cnt = 0;cnt <self.initRegisterArray.length;cnt ++){
                console.log("thread."+cnt);
                i2cSlave.write8(self.initRegisterArray[cnt][0],self.initRegisterArray[cnt][1]);
                yield self.sleep(10, thread2);
              }
              i2cSlave.write8(0xef,0);  // BankSelect=0
              yield self.sleep(10, thread2);
  ...
  read : function(){
    ...
    return new Promise(function(resolve, reject){
      self.i2cPort.open(self.slaveAddress).then(function(i2cSlave){
        i2cSlave.read8(0x43,true).then(function(v){
          var res;
          if(v == self.GES_RIGHT_FLAG){
            res = "right";
          }else if(v == self.GES_LEFT_FLAG){
            res = "left";
          }else if(v == self.GES_UP_FLAG){
            res = "up";
          }else if(v == self.GES_DOWN_FLAG){
            res = "down";
          }else if(v == self.GES_FORWARD_FLAG){
            res = "forward";
          }else if(v == self.GES_BACKWARD_FLAG){
            res = "back";
          }else if(v == self.GES_CLOCKWISE_FLAG){
            res = "clockwise";
          }else if(v == self.GES_COUNT_CLOCKWISE_FLAG){
            res = "count clockwise";
          }else{
            res = "----";
          }
