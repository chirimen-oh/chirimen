...
var seesaw = function(i2cPort,slaveAddress){
	...
	this.NP_PIXEL_ORDERS=[[0,1,2],[1,0,2],[0,1,2,3],[1,0,2,3]];
	this.NP_RGB=0;
...
seesaw.prototype = {
	...
	sw_reset: async function(){
		...
		var pid = (await this.readReg32(this._STATUS_BASE, this._STATUS_VERSION)) >> 16;
...
//			await this.pin_mode_bulk_b(1 << (pin - 32), mode);
	...
	_pin_mode_bulk_x: async function( capacity, offset, pins, mode){
		...
		if ( capacity != 4 ){ // only support portA
			return;
		}
...
//		console.log("digital_read_bulk1:",buf.toString(16));
	...
	analog_read: async function(pin){
		...
		var buf = this.readReg16(this._ADC_BASE, this._ADC_CHANNEL_OFFSET + pinIndex);
		await sleep(1);
		return (buf);
	},
	analog_write: async function(pin, value){ // PWM_write 0..255
		var pin_found = false;
		var cmd;
	...
	get_temp: async function(){ // NOT SUPPORTED SAMD09 always -1
	...
	set_pwm_freq: async function(pin, freq){ // NOT SUPPORTED SAMD09 Fixed 2.87KHz?
		var pinIndex = this.inArray(pin , this.pin_mapping.pwm_pins);
		if (pinIndex >= 0 ){
...
//		console.log("initNeopixel:pin:",pin," quantity:",quantity," auto_write:",auto_write);
		...
		if (pixel_order != undefined && 0 <= pixel_order && pixel_order <= NP_PIXEL_ORDERS.length){
...
//		console.log("setPixel:",n,r,g,b);
	...
	setPixel_int: async function(n,c){
		...
		var n_l = bn & 0xFF;
		if ( this._bpp == 3 ){
			await this.writeRegN(this._NEOPIXEL_BASE, this._NEOPIXEL_BUF, [n_h, n_l, c[this._pixel_order[0]],c[this._pixel_order[1]],c[this._pixel_order[2]]]);
		} else {
			// untested code (bpp:4 rgbw,grbw devices)
			await this.writeRegN(this._NEOPIXEL_BASE, this._NEOPIXEL_BUF, [n_h, n_l, c[this._pixel_order[0]],c[this._pixel_order[1]],c[this._pixel_order[2]],c[this._pixel_order[3]]]);
		}
	},
...
//			console.log("call setPixs_int Last :ofs:",ofs,"  ca:",ca);
	...
	fillPixels1:async function(r,g,b){ // 1pixづつ送るタイプ(基本obsolute)
		var c = this.getColor(r,g,b);
...
//		console.log("showPixels");
	...
	readReg8: async function(baseAddr, funcAddr, delay){
		...
		if ( delay ){ await sleep(delay)} else { await sleep(8)}
...
//		console.log("writeReg8: B:",baseAddr.toString(16)," F:",funcAddr.toString(16)," D:",data8.toString(16));
		await this.i2cSlave.writeBytes([(baseAddr & 0xFF), (funcAddr & 0xFF), (data8 & 0xFF)]);
	},
...
//		console.log("writeReg16: B:",baseAddr.toString(16)," F:",funcAddr.toString(16), " DH:",((data16 >>> 8) & 0xFF).toString(16), " DL:",(data16 & 0xFF).toString(16));
		await this.i2cSlave.writeBytes([(baseAddr & 0xFF), (funcAddr & 0xFF), ((data16 >>> 8) & 0xFF), (data16 & 0xFF)]);
	},
	...
	writeReg24: async function(baseAddr, funcAddr, data24){
		await this.i2cSlave.writeBytes([(baseAddr & 0xFF), (funcAddr & 0xFF), ((data24 >>>16) & 0xFF), ((data24 >>> 8) & 0xFF), (data24 & 0xFF)]);
	},
...
//		console.log("writeReg32:",baseAddr.toString(16),funcAddr.toString(16),data32.toString(16));
		await this.i2cSlave.writeBytes([(baseAddr & 0xFF), (funcAddr & 0xFF), (data32 >>> 24), ((data32 >>>16) & 0xFF), ((data32 >>> 8) & 0xFF), (data32 & 0xFF)]);
	},
