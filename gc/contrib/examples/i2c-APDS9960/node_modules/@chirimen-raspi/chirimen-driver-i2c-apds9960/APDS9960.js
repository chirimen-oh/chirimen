...
//		this.bus = bus;
		...
		for ( var i = 0 ; i < (this.APDS9960_DEV_ID).length ;  i++ ){
...
//		console.log("this.dev_id:",this.dev_id, this.dev_id.toString(16),devIdInvalid,this.APDS9960_DEV_ID,(this.APDS9960_DEV_ID).length);
		...
		await this.i2cSlave.write8(this.APDS9960_REG_ATIME, this.APDS9960_DEFAULT_ATIME);
		await this.i2cSlave.write8(this.APDS9960_REG_WTIME, this.APDS9960_DEFAULT_WTIME);
		await this.i2cSlave.write8(this.APDS9960_REG_PPULSE, this.APDS9960_DEFAULT_PROX_PPULSE);
		await this.i2cSlave.write8(this.APDS9960_REG_POFFSET_UR, this.APDS9960_DEFAULT_POFFSET_UR);
		await this.i2cSlave.write8(this.APDS9960_REG_POFFSET_DL, this.APDS9960_DEFAULT_POFFSET_DL);
		await this.i2cSlave.write8(this.APDS9960_REG_CONFIG1, this.APDS9960_DEFAULT_CONFIG1);
		...
		await this.i2cSlave.write8(this.APDS9960_REG_PERS, this.APDS9960_DEFAULT_PERS);
		await this.i2cSlave.write8(this.APDS9960_REG_CONFIG2, this.APDS9960_DEFAULT_CONFIG2);
		await this.i2cSlave.write8(this.APDS9960_REG_CONFIG3, this.APDS9960_DEFAULT_CONFIG3);
		...
		await this.i2cSlave.write8(this.APDS9960_REG_GCONF1, this.APDS9960_DEFAULT_GCONF1);
		...
		await this.i2cSlave.write8(this.APDS9960_REG_GOFFSET_U, this.APDS9960_DEFAULT_GOFFSET);
		await this.i2cSlave.write8(this.APDS9960_REG_GOFFSET_D, this.APDS9960_DEFAULT_GOFFSET);
		await this.i2cSlave.write8(this.APDS9960_REG_GOFFSET_L, this.APDS9960_DEFAULT_GOFFSET);
		await this.i2cSlave.write8(this.APDS9960_REG_GOFFSET_R, this.APDS9960_DEFAULT_GOFFSET);
		await this.i2cSlave.write8(this.APDS9960_REG_GPULSE, this.APDS9960_DEFAULT_GPULSE);
		await this.i2cSlave.write8(this.APDS9960_REG_GCONF3, this.APDS9960_DEFAULT_GCONF3);
...
//		console.log("set reg_val:",reg_val);
	...
	enableLightSensor: async function( interrupts){
		if ( interrupts==undefined){interrupts=true}
//		console.log("enableLightSensor:",this.APDS9960_DEFAULT_AGAIN,interrupts,this.APDS9960_MODE_AMBIENT_LIGHT);
	...
	enableGestureSensor: async function(interrupts){
		...
		await this.i2cSlave.write8(this.APDS9960_REG_PPULSE, this.APDS9960_DEFAULT_GESTURE_PPULSE);
	...
	isGestureAvailable: async function(){ // Gesture Status Register
...
//		console.log("isGestureAvailable:",val);
	...
	zeroGesture: async function(){
		...
		**/
		...
		if (fifo_level > 0){
			fifo_data = [];
			for (var i =0; i <  fifo_level;i++){
				// _read_i2c_block_dataを
				fifo_data.push( await this._read_i2c_block_data(this.APDS9960_REG_GFIFO_U, 4));
			}
...
//				var zc = new Array(4);
				...
				for ( var i = 0 ; i <fifo_data.length ; i++){
					for ( var j = 0 ; j < 4 ; j++){
...
//			console.log("zeroGesture:",this.GestureZero);
				for ( var j = 0 ; j < 4 ; j++){
					this.GestureZero[j] = Math.floor(this.GestureZero[j] / fifo_data.length);
				}
			}
		}
		console.log("this.GestureZero:",this.GestureZero);
	},
	readGesture: async function(){ // readGestureはあまり良い動きをしていません・・・
		var fifo_level = 0;
		var bytes_read = 0;
		var fifo_data = [];
		var motion;
		...
		if (fifo_level > 0){
			...
			for (var i =0; i <  fifo_level;i++){
				// _read_i2c_block_dataを
				var rawData = await this._read_i2c_block_data(this.APDS9960_REG_GFIFO_U, 4);
				for ( var j = 0 ; j < 4 ; j++ ){
	...
	surveyGesture: function(fifo_data){ // readGestureのgesture判定ルーチン。まだかなり不完全だと思います・・・
		var startDirX = 0;
		var startDirY = 0;
		var noEdgeX = false;
		var noEdgeY = false;
		...
		for ( var i = 0 ; i < fifo_data.length ; i++ ){
			...
			if ( xtot > 70 ){ // 両方センサー検知があるかどうかを見ている
				if ( i== 0 ){ // バッファの開始時点で両方信号があった場合、Xのスタート検知ができてない
					noEdgeX = true;
				} 
				if ( !noEdgeX && startDirX == 0){ // スタート検知出来ていてまだ方向が決まってなければ、方向を決める
					startDirX = (xdif);
				}
				if ( startDirX * xdif > 0 && Math.abs(startDirX) < Math.abs(xdif) ){ // 検知の方向と同じ方向の信号の最大値を蓄える
					startDirX =  (xdif)
				}
			}
			...
			if ( ytot > 70 ){
				...
				if ( startDirY * ydif > 0 && Math.abs(startDirY) < Math.abs(ydif) ){ // 検知の方向と同じ方向の信号の最大値を蓄える
					startDirY =  (ydif)
				}
			}
		}
		...
		noEdgeY = false;
		// 終了方向を見るパターン
		for ( var i = fifo_data.length-1 ; i >= 0 ; i-- ){
			var ydif = fifo_data[i][0] - fifo_data[i][1];
			var xdif = fifo_data[i][2] - fifo_data[i][3];
			var ytot = fifo_data[i][0] + fifo_data[i][1];
			var xtot = fifo_data[i][2] + fifo_data[i][3];
			...
			if ( xtot > 70 ){ // 両方センサー検知があるかどうかを見ている
				if ( i== fifo_data.length-1 ){ // バッファの開始時点で両方信号があった場合、Xのスタート検知ができてない
					noEdgeX = true;
				} 
				if ( !noEdgeX && endDirX == 0){ // スタート検知出来ていてまだ方向が決まってなければ、方向を決める
					endDirX = (xdif);
				}
				if ( endDirX * xdif > 0 && Math.abs(endDirX) < Math.abs(xdif) ){ // 検知の方向と同じ方向の信号の最大値を蓄える
					endDirX =  (xdif)
				}
			}
			...
			if ( ytot > 70 ){
				...
				if ( endDirY * ydif > 0 && Math.abs(endDirY) < Math.abs(ydif) ){ // 検知の方向と同じ方向の信号の最大値を蓄える
					endDirY =  (ydif);
				}
			}
		}
...
//		console.log(hist);
		...
		if ( Math.abs(Math.abs(startDirX) - Math.abs(startDirY))<30 ){ // XY方向で差が小さい場合　判別不能とする？
			ans1 ="none";
		...
		var ans2;
		var ans2str = 0;
		if ( Math.abs(Math.abs(endDirX) - Math.abs(endDirY))<30 ){ // XY方向で差が小さい場合　判別不能とする？
			ans2 ="none";
		...
		console.log("DIR:", ans, " dir1,2:",ans1  , ans2 , "  move_s x,y:",startDirX,startDirY, "  move_e x,y:",endDirX,endDirY);
		return ( ans );
	},
...
//		console.log("readAmbientLight:",l,h);
	...
	resetGestureParameters: function(){ //NOT ASYNC FUNC
	...
	processGestureData: function(){ //NOT ASYNC FUNC
		var u_first = 0;
		var d_first = 0;
		var l_first = 0;
		var r_first = 0;
		...
		if (this.gesture_data_.total_gestures <= 32 && this.gesture_data_.total_gestures > 0){
			// find the first value in U/D/L/R above the threshold
			for (var i = 0 ; i < this.gesture_data_.total_gestures; i++){
				if (this.gesture_data_.u_data[i] > this.APDS9960_GESTURE_THRESHOLD_OUT &&
			...
			for ( var i = this.gesture_data_.total_gestures -1 ; i >= 0 ; i--){
				if (this.gesture_data_.u_data[i] > this.APDS9960_GESTURE_THRESHOLD_OUT &&
					this.gesture_data_.d_data[i] > this.APDS9960_GESTURE_THRESHOLD_OUT &&
					this.gesture_data_.l_data[i] > this.APDS9960_GESTURE_THRESHOLD_OUT &&
					this.gesture_data_.r_data[i] > this.APDS9960_GESTURE_THRESHOLD_OUT){
...
//			console.log("ud_delta:",this.gesture_ud_delta_,"  lr_delta:",this.gesture_lr_delta_);
			...
			} else if (this.gesture_ud_delta_ <= -this.APDS9960_GESTURE_SENSITIVITY_1){
				this.gesture_ud_count_ = -1;
			} else{
				this.gesture_ud_count_ = 0;
			}
			...
			}else if (this.gesture_lr_delta_ <= -this.APDS9960_GESTURE_SENSITIVITY_1){
				this.gesture_lr_count_ = -1;
			}else{
				this.gesture_lr_count_ = 0;
			}
			console.log("ud_delta:",this.gesture_ud_delta_," lr_delta:",this.gesture_lr_delta_);
			console.log("ud_count:",this.gesture_ud_count_,"  lr_count:",this.gesture_lr_count_ );
				
			// determine Near/Far gesture
			if (this.gesture_ud_count_ == 0 && this.gesture_lr_count_ == 0){
				if (Math.abs(ud_delta) < this.APDS9960_GESTURE_SENSITIVITY_2 && Math.abs(lr_delta) < this.APDS9960_GESTURE_SENSITIVITY_2){
				...
				if (Math.abs(ud_delta) < this.APDS9960_GESTURE_SENSITIVITY_2 &&
...
//		console.log("FALSE???");
	...
	decodeGesture: function(){ //NOT ASYNC FUNC
...
//		console.log("setAmbientLightGain:",drive,this.APDS9960_REG_CONTROL, val);
	...
	getLightIntLowThreshold: async function(){
		return (await this.i2cSlave.read8(this.APDS9960_REG_AILTL) | (await this.i2cSlave.read8(this.APDS9960_REG_AILTH) << 8));
	},
	setLightIntLowThreshold: async function( threshold){
		...
		await this.i2cSlave.write8(this.APDS9960_REG_AILTH, (threshold & 0xff00) >> 8);
	},
	getLightIntHighThreshold: async function(){
		return (await this.i2cSlave.read8(this.APDS9960_REG_AIHTL) | (await this.i2cSlave.read8(this.APDS9960_REG_AIHTH) << 8));
	},
	setLightIntHighThreshold: async function( threshold){
		// break 16-bit threshold into 2 8-bit values
		await this.i2cSlave.write8(this.APDS9960_REG_AIHTL, threshold & 0x00ff);
		await this.i2cSlave.write8(this.APDS9960_REG_AIHTH, (threshold & 0xff00) >> 8);
	},
