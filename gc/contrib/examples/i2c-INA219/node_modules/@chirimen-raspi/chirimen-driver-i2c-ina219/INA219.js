...
var INA219 = function(i2cPort,slaveAddress){
	...
	this.__RNG_ERR_MSG = ('Expected amps , out of range, use a lower value shunt resistor');
	this.__VOLT_ERR_MSG = ('Invalid voltage range, must be one of: RANGE_16V, RANGE_32V')

//	this.__LOG_FORMAT = '%(asctime)s - %(levelname)s - INA219 %(message)s';
	this.__LOG_MSG_1 = ('shunt ohms: , bus max volts: , shunt volts max: , bus ADC: , shunt ADC: ');
	this.__LOG_MSG_2 = ('calibrate called with: bus max volts: , max shunt volts, max_expected_amps: ');
...
INA219.prototype = {
	...
	configure: async function(voltage_range, gain, bus_adc, shunt_adc){
		...
		console.log('gain set to ', this.__GAIN_VOLTS[this._gain], " auto_gain_enabled:",this._auto_gain_enabled);
		
		await this._calibrate(
		    this.__BUS_RANGE[voltage_range], this.__GAIN_VOLTS[this._gain],
...
//		console.log("gain:",gainIndex+1);
	...
	_increase_gain: async function(){
		...
		if (gain < (this.__GAIN_VOLTS).length - 1){
			gain = gain + 1;
			await this._calibrate(this.__BUS_RANGE[this._voltage_range],this.__GAIN_VOLTS[gain]);
	...
	_configure: async function( voltage_range, gain, bus_adc, shunt_adc){
		var configuration = (
			voltage_range << this.__BRNG | gain << this.__PG0 |
			bus_adc << this.__BADC1 | shunt_adc << this.__SADC1 |
			this.__CONT_SH_BUS);
		await this._configuration_register(configuration);
	},
	_calibrate: async function( bus_volts_max, shunt_volts_max, max_expected_amps){
		console.log(this.__LOG_MSG_2 ,bus_volts_max, shunt_volts_max, (max_expected_amps));
		var max_possible_amps = shunt_volts_max / this._shunt_ohms;
		console.log("max possible current:",max_possible_amps);

		this._current_lsb = this._determine_current_lsb(max_expected_amps, max_possible_amps);
		...
		console.log("max shunt voltage before overflow: mV ;",(max_shunt_voltage * 1000));

		var calibration = Math.trunc(this.__CALIBRATION_FACTOR / (this._current_lsb * this._shunt_ohms));
...
//		console.log("_determine_current_lsb:",max_expected_amps, max_possible_amps);
		...
		if (max_expected_amps){
			if (max_expected_amps > (max_possible_amps)){
				throw("ValueError: " + this.__AMP_ERR_MSG +" : " + max_expected_amps + " : " + max_possible_amps);
	...
	_calculate_min_current_lsb: function(){
		return this.__CALIBRATION_FACTOR / (this._shunt_ohms * this.__MAX_CALIBRATION_VALUE);
	},
	...
	__write_register: async function( register, register_value){
		...
		console.log("write register :" ,register, register_value, register_value.toString(2));
